...
def is_valid_oklahoma_zip(zip: str):
    list_of_dicts_of_oklahoma_zips = zipcodes.filter_by(state="OK")
    list_of_oklahoma_zips = [d["zip_code"] for d in list_of_dicts_of_oklahoma_zips]
    return len(zip) <= 10 and type(zip) == str and zip in list_of_oklahoma_zips
...
class ColumnValuesToBeValidOklahomaZip(ColumnMapMetricProvider):
    ...
    def _pandas(cls, column, **kwargs):
        return column.apply(lambda x: is_valid_oklahoma_zip(x))
...
class ExpectColumnValuesToBeValidOklahomaZip(ColumnMapExpectation):
