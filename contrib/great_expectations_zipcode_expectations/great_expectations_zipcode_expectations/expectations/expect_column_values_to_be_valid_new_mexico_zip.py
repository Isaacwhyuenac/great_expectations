...
def is_valid_new_mexico_zip(zip: str):
    list_of_dicts_of_new_mexico_zips = zipcodes.filter_by(state="NM")
    list_of_new_mexico_zips = [d["zip_code"] for d in list_of_dicts_of_new_mexico_zips]
    return len(zip) <= 10 and type(zip) == str and zip in list_of_new_mexico_zips


# This class defines a Metric to support your Expectation.
# For most ColumnMapExpectations, the main business logic for calculation will live in this class.
class ColumnValuesToBeValidNewMexicoZip(ColumnMapMetricProvider):
    ...
    def _pandas(cls, column, **kwargs):
        return column.apply(lambda x: is_valid_new_mexico_zip(x))
