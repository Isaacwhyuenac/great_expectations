...
class DataProfilerProfileNumericColumnsDiffGreaterThanThreshold(
    ...
    metric_name = "data_profiler.profile_numeric_columns_diff_greater_than_threshold"
        ...
        for col, stats in limit_check_report_keys_copy.items():
            ...
            for stat, threshold in stats.items():
                ...
                if diff_val == "unchanged":  # In the case there is no delta
                    diff_val = 0
                if is_greater := is_value_greater_than_threshold(
                    diff_val, threshold
                ):
    ...
    def _get_evaluation_dependencies(
        ...
        if (
            metric.metric_name
            == "data_profiler.profile_numeric_columns_diff_greater_than_threshold"
        ):
...
class ExpectProfileNumericColumnsDiffGreaterThanThreshold(
    ...
    profile_metric = "data_profiler.profile_numeric_columns_diff_greater_than_threshold"
...
if __name__ == "__main__":
    diagnostics_report = (
        ExpectProfileNumericColumnsDiffGreaterThanThreshold().run_diagnostics()
    )
